...
public abstract class MmsRequest {
    ...
    public void execute(Context context, MmsNetworkManager networkManager) {
        ...
        if (!useWifi(context)) {
//            android.util.Log.d(TAG, "와이파이 연결되있음");
//            wifi.setWifiEnabled(false);//16.05.04 wifi 끄기
        }

        mobileDataEnabled = Utils.isMobileDataEnabled(context);
        Log.v(TAG, "mobile data enabled: " + mobileDataEnabled);
        android.util.Log.v(TAG, "mobile data enabled: " + mobileDataEnabled);

        //TEST
        if (!mobileDataEnabled && !useWifi(context)) {
            Log.v(TAG, "mobile data not enabled, so forcing it to enable");
            android.util.Log.v(TAG, "mobile data not enabled, so forcing it to enable");
            ...
            for (int i = 0; i < RETRY_TIMES; i++) {
                try {
                    try {
                        ...
                        android.util.Log.e(TAG, "error acquiring network", e);
                    }
                    ...
                    try {
                        ...
                        try {
                            ...
                            android.util.Log.d(TAG, "MmsRequest: No match with APN name:"
                                    + apnName + ", try with no name");
                            apn = ApnSettings.load(context, null, mSubId);
                        }
                        Log.i(TAG, "MmsRequest: using " + apn.toString());
                        android.util.Log.d(TAG, "MmsRequest: using " + apn.toString());
                    ...
                    android.util.Log.e(TAG, "MmsRequest: APN failure", e);
...
//                        // Retry
                    ...
                    android.util.Log.e(TAG, "MmsRequest: HTTP or network I/O failure", e);
                    result = SmsManager.MMS_ERROR_HTTP_FAILURE;
                    httpStatusCode = e.getStatusCode();
                    // Retry
                } catch (Exception e) {
                    Log.e(TAG, "MmsRequest: unexpected failure", e);
                    android.util.Log.e(TAG, "MmsRequest: unexpected failure", e);
                    android.util.Log.e(TAG, "MmsRequest: unexpected failure", e);
