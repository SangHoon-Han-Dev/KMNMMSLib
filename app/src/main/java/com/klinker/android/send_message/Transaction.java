...
public class Transaction {
    ...
    private boolean saveMessage = true;//메시지 저장
//    private boolean saveMessage = false;
    ...
    public void sendNewMessage(Message message, long threadId) {
        ...
        if (checkMMS(message)) {
//        if (checkMMS(message) && message.getMessage_type() == Message.TYPE_MESSAGE_TYPE_MMS) {
            android.util.Log.d(TAG, "sending mms");
            try {
                Looper.prepare();
            } catch (Exception e) {
            }
            Log.v("send_transaction", "sending mms");
            RateController.init(context);
            DownloadManager.init(context);

            //TEST
            sendMmsMessage2(message.getText(), message.getAddresses(), message.getImages(), message.getImageNames(), message.getParts(), message.getSubject());
//            sendMmsMessage(message.getText(), message.getAddresses(), message.getImages(), message.getImageNames(), message.getParts(), message.getSubject());
    ...
    private void sendSmsMessage(String text, String[] addresses, long threadId, int delay) {
        ...
        android.util.Log.d("send_transaction", "message text: " + text);
        Uri messageUri = null;
        int messageId = 0;
        if (saveMessage) {
            Log.v("send_transaction", "saving message");
            android.util.Log.d("send_transaction", "saving message");
            ...
            for (int i = 0; i < addresses.length; i++) {
                ...
                android.util.Log.d("send_transaction", "saving message with thread id: " + threadId);
                ...
                android.util.Log.d("send_transaction", "inserted to uri: " + messageUri);
                ...
                android.util.Log.d("send_transaction", "message id: " + messageId);
                ...
                android.util.Log.d("send_transaction", "found sms manager");

                if (settings.getSplit()) {
                    Log.v("send_transaction", "splitting message");
                    android.util.Log.d("send_transaction", "splitting message");
                    ...
                    android.util.Log.d("send_transaction", "length: " + length);
                    ...
                    for (int j = 0; j < textToSend.length; j++) {
                        ...
                        android.util.Log.d("send_transaction", "sending split message");
                        sendDelayedSms(smsManager, addresses[i], parts, sPI, dPI, delay, messageUri);
                    }
                } else {
                    Log.v("send_transaction", "sending without splitting");
                    android.util.Log.d("send_transaction", "sending without splitting");
                    ...
                    try {
                        ...
                        android.util.Log.d("send_transaction", "sent message");
                        sendDelayedSms(smsManager, addresses[i], parts, sPI, dPI, delay, messageUri);
                    } catch (Exception e) {
                        // whoops...
                        Log.v("send_transaction", "error sending message");
                        android.util.Log.d("send_transaction", "error sending message");
                        Log.e(TAG, "exception thrown", e);
                        android.util.Log.d(TAG, "exception thrown", e);
            ...
            Log.v("send_transaction", "non saving message");
            android.util.Log.d("send_transaction", "non saving message");
            // add signature to original text to be saved in database (does not strip unicode for saving though)
            if (!settings.getSignature().equals("")) {
                text += "\n" + settings.getSignature();
            }
    ...
    private void sendDelayedSms(final SmsManager smsManager, final String address,
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                smsManager.sendMultipartTextMessage(address, null, parts, sPI, dPI);//TEST
                try {
                    Thread.sleep(delay);
                } catch (Exception e) {
                }
                ...
                if (checkIfMessageExistsAfterDelay(messageUri)) {
                    ...
                    android.util.Log.d("send_transaction", "message sent after delay");
                    try {
                    ...
                    android.util.Log.d("send_transaction", "message not sent after delay, no longer exists");
                }
            }
        }).start();
    }
    ...
    //TEST
    private void sendMmsMessage2(String text, String[] addresses, Bitmap[] image, String[] imageNames, List<Message.Part> parts, String subject) {
        android.util.Log.d(TAG, "sendMmsMessage");
        ...
        for (int i = 0; i < image.length; i++) {
            ...
            String empty = " ";
            MMSPart part_empty = new MMSPart();
            part_empty.Name = "text1";
            part_empty.MimeType = "text/plain";
            part_empty.Data = empty.getBytes();
            data.add(part_empty);
        }
        android.util.Log.d(TAG, text + address + imageNames + subject);
        Log.v(TAG, text + address + imageNames + subject);
        ...
        if (parts != null) {
            for (Message.Part p : parts) {
                ...
                Log.v(TAG, "PART MMSPART" + "namd:" + part.Name + " // mimetype:" + part.MimeType + " // ");
                data.add(part);
            }
        }
        ...
        if (!text.equals("")) {
            ...
            Log.v(TAG, "text MMSPART" + "namd:" + part.Name + " // mimetype:" + part.MimeType + " // ");
            android.util.Log.d(TAG, "text MMSPART" + "namd:" + part.Name + " // mimetype:" + part.MimeType + " // ");
            data.add(part);
        }

        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.KITKAT) {
            android.util.Log.d(TAG, "KITKAT 이하");
            ...
            try {
                ...
                BroadcastReceiver receiver = new BroadcastReceiver() {
                    ...
                    public void onReceive(Context context, Intent intent) {
                        ...
                        android.util.Log.d("sending_mms_library", "progress: " + progress);
                        ...
                        if (progress == ProgressCallbackEntity.PROGRESS_COMPLETE) {
                            ...
                            android.util.Log.d("sending_mms_library", "sending aborted for some reason...");
                        }
                    }
            ...
            android.util.Log.d(TAG, "KITKAT 이상");
            android.util.Log.d(TAG, "using lollipop method for sending sms");
            Log.v(TAG, "using lollipop method for sending sms");

            if (!settings.getUseSystemSending()) {
                android.util.Log.d(TAG, "using system method for sending");
                Log.v(TAG, "using system method for sending");
                sendMmsThroughSystem(context, subject, data, addresses);
            } else {
                try {
                    android.util.Log.d(TAG, "using system method for sending             else");
                    ...
                    android.util.Log.d(TAG, "error sending mms", e);
                }
            }
        }
    }
    ...
    private void sendMMS(final byte[] bytesToSend) {
        android.util.Log.d(TAG, "sendMMS");
        revokeWifi(true);
    ...
    private void sendMMSWiFi(final byte[] bytesToSend) {
        android.util.Log.d(TAG, "sendMMSWiFi");
        // enable mms connection to mobile data
        mConnMgr = (ConnectivityManager) context.getSystemService(Context.CONNECTIVITY_SERVICE);
